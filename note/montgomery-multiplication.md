...
- By defninition, $\overline{x} = x \cdot r \mod{p}$ where $r=2^{31}$ and $2^{30} < p < 2^{31}$, which impiles $\overline{x} < 2^{31}$.
  ...
  - so, `- p` of `d = (uint32_t)((z + w) >> 31) - p;` is equivalent to **mod p** of $( (\overline{x} \cdot \overline{y} + ((-\overline{x} \cdot \overline{y} \cdot p^{-1}) \mod{r}) \cdot p) \cdot r^{-1} ) \textbf{mod } \textbf{p}$
