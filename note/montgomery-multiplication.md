...
So ```& (uint64_t)0x7FFFFFFF``` is equivalent to $\boldsymbol{\mod{r}}$ sine $r$ is pow 2 31 ($2^{31}$), masking $(-\overline{x} \cdot \overline{y} \cdot p^{-1})$ with ```0x7FFFFFFF``` means getting the remainings divided by $2^{31}$.
